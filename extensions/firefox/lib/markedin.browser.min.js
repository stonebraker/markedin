...
`&&i++,{lineStart:t,lineEnd:i}}function Fo(e,r,n,t){let i=1,o=t;for(;i>0;){let l=e.indexOf(n,o);if(l===-1)return-1;r.lastIndex=o;let a;for(;(a=r.exec(e))!==null&&a.index<l;)i++;if(--i===0)return l;o=l+n.length}return-1}function Oo(e){let r=/\{\{#if [\w.[\]]+\}\}|\{\{\/if\}\}|\{\{else\}\}/g,n=0,t;for(;(t=r.exec(e))!==null;)if(t[0].startsWith("{{#if "))n++;else if(t[0]==="{{/if}}")n--;else if(t[0]==="{{else}}"&&n===0)return t.index;return-1}function ii(e,r,n,t,i){let o="",l=0,a;for(;(a=r.exec(e))!==null;){let s=a.index,u=a.index+a[0].length,c=Fo(e,n,t,u);if(c===-1)continue;let p=c+t.length,h=s,f=p,d=u,x=c,w=mr(e,s,u);w&&(h=w.lineStart,d=w.lineEnd);let S=mr(e,c,p);S&&(f=S.lineEnd);let m=e.slice(d,x);o+=e.slice(l,h),o+=i(a[1],m),l=f,r.lastIndex=l}return o+e.slice(l)}function oi(e,{embed:r=!1}={}){let{data:n,body:t}=br(e),i=pe(t,n);return r&&(i=i.trimEnd()+`

<!-- frontmatter
`+JSON.stringify(n,null,2)+`
-->
`),i}function qo(e){let r=oi(e);return kr.parse(r,{gfm:!0})}function $o(e,{embed:r=!1}={}){let{data:n,body:t}=br(e),i=pe(t,n),o=kr.parse(i,{gfm:!0}),l=n.title||"",a="";return r&&(a=`
<script type="application/json" id="frontmatter">
`+JSON.stringify(n,null,2)+`
<\/script>`),No.replace("%TITLE%",l).replace("%DATA_BLOCK%",a).replace("%BODY%",o)}var No=`<!doctype html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
</html>`,xr,ti;function pe(e,r,n){let t=n!==!1;t&&(xr=new Map,ti=0);function i(a){let s=`${ti++}`;return xr.set(s,a),s}function o(a){let s=a;for(let[u,c]of[...xr.entries()].reverse())s=s.split(u).join(c);return s}let l=e;return l=l.replace(/\\\{\{/g,()=>i("{{")),l=ii(l,/\{\{#each ([\w.[\]]+)\}\}/g,/\{\{#each [\w.[\]]+\}\}/g,"{{/each}}",(a,s)=>{let u=ce(r,a);return Array.isArray(u)?i(u.map((c,p)=>{let h={this:c,"@index":p,"@first":p===0,"@last":p===u.length-1};return c&&typeof c=="object"&&!Array.isArray(c)&&Object.assign(h,c),pe(s,h,!1)}).join("")):i("")}),l=ii(l,/\{\{#if ([\w.[\]]+)\}\}/g,/\{\{#if [\w.[\]]+\}\}/g,"{{/if}}",(a,s)=>{let u=ce(r,a),c=Oo(s),p,h;if(c===-1)p=s,h="";else{let f=c+8,d=mr(s,c,f);d?(p=s.slice(0,d.lineStart),h=s.slice(d.lineEnd)):(p=s.slice(0,c),h=s.slice(f))}return i(pe(Io(u)?p:h,r,!1))}),l=l.replace(/\{\{> ?([\w.[\]]+)\}\}/g,(a,s)=>{let u=ce(r,s);return u!=null?i(String(u)):""}),l=l.replace(/\{\{([\w.[\]@]+)\}\}/g,(a,s)=>{let u=ce(r,s);return u==null?"":i(Lo(u))}),t?o(l):l}si.exports={SPEC_VERSION:Co,parse:br,render:oi,renderHtmlFrag:qo,renderHtml:$o,renderTemplate:pe,resolvePath:ce,yaml:li,marked:Eo}});return Po();})();
